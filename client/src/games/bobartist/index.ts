...
app.innerHTML = `
  <main id="lobbyPage" class="page">
    <section class="panel">
      ...
      <div class="lobby-settings" aria-label="게임 시간 설정">
        <label>
          숨는 시간
          <select id="decorateTimeSelect">
        ...
        <label>
          찾는 시간
          <select id="findTimeSelect">
            <option value="180000">3분</option>
            <option value="300000" selected>5분</option>
            <option value="420000">7분</option>
          </select>
        </label>
        <p id="settingsHint" class="settings-hint">방장이 선택한 시간으로 게임이 시작됩니다. 첫 술래는 랜덤입니다.</p>
      </div>
    ...
    <section class="game-shell">
      ...
      <section class="game-layout">
        ...
        <aside class="mission-card">
          ...
          <p class="hint">v0.0.59은 BobPlatform에서 기존 은신 순위 기능을 유지합니다.</p>
        </aside>
      </section>
    </section>
  </main>
...
const roomCodeInput =
  document.querySelector<HTMLInputElement>("#roomCodeInput")!;
const artworkInput = document.querySelector<HTMLInputElement>("#artworkInput")!;
const decorateTimeSelect = document.querySelector<HTMLSelectElement>(
  "#decorateTimeSelect",
)!;
const findTimeSelect =
  document.querySelector<HTMLSelectElement>("#findTimeSelect")!;
const settingsHint =
  document.querySelector<HTMLParagraphElement>("#settingsHint")!;
const uploadInfo = document.querySelector<HTMLDivElement>("#uploadInfo")!;
const createRoomButton =
  document.querySelector<HTMLButtonElement>("#createRoomButton")!;
const joinRoomButton =
  document.querySelector<HTMLButtonElement>("#joinRoomButton")!;
const leaveRoomButton =
  document.querySelector<HTMLButtonElement>("#leaveRoomButton")!;
...
const gameSubTitle =
  document.querySelector<HTMLParagraphElement>("#gameSubTitle")!;
const gameRoomCode = document.querySelector<HTMLSpanElement>("#gameRoomCode")!;
const gameLeaveButton =
  document.querySelector<HTMLButtonElement>("#gameLeaveButton")!;
...
const gameCanvasViewport = document.querySelector<HTMLDivElement>(
  "#gameCanvasViewport",
)!;
const spyVisionOverlay =
  document.querySelector<HTMLDivElement>("#spyVisionOverlay")!;
...
const resetCharacterButton = document.querySelector<HTMLButtonElement>(
  "#resetCharacterButton",
)!;
const zoomView = document.querySelector<HTMLSpanElement>("#zoomView")!;
const brushSizeInput =
  document.querySelector<HTMLInputElement>("#brushSizeInput")!;
const brushSizeView = document.querySelector<HTMLElement>("#brushSizeView")!;
const characterSizeInput = document.querySelector<HTMLInputElement>(
  "#characterSizeInput",
)!;
const characterSizeView =
  document.querySelector<HTMLElement>("#characterSizeView")!;
const roleCard = document.querySelector<HTMLElement>("#roleCard")!;
const myRoleView = document.querySelector<HTMLElement>("#myRoleView")!;
const roleDescriptionView = document.querySelector<HTMLParagraphElement>(
  "#roleDescriptionView",
)!;
const selectedColorChip =
  document.querySelector<HTMLSpanElement>("#selectedColorChip")!;
const selectedColorText =
  document.querySelector<HTMLElement>("#selectedColorText")!;
...
const gameArtworkNameView = document.querySelector<HTMLElement>(
  "#gameArtworkNameView",
)!;
const gamePlayerCountView = document.querySelector<HTMLElement>(
  "#gamePlayerCountView",
)!;
const focusScorePanel =
  document.querySelector<HTMLElement>("#focusScorePanel")!;
const focusScoreLabel =
  document.querySelector<HTMLElement>("#focusScoreLabel")!;
const focusScoreView = document.querySelector<HTMLElement>("#focusScoreView")!;
const submitArtworkButton = document.querySelector<HTMLButtonElement>(
  "#submitArtworkButton",
)!;
const confirmFindButton =
  document.querySelector<HTMLButtonElement>("#confirmFindButton")!;
const restartGameButton =
  document.querySelector<HTMLButtonElement>("#restartGameButton")!;
const submitStatusView =
  document.querySelector<HTMLParagraphElement>("#submitStatusView")!;

if (!lobbyContext || !gameContext)
...
function getSelectedGameSettings(): GameSettings {
  return {
    decorateDurationMs:
      Number(decorateTimeSelect.value) || DEFAULT_DECORATE_DURATION_MS,
    findDurationMs: Number(findTimeSelect.value) || DEFAULT_FIND_DURATION_MS,
  };
}
...
function syncSettingsSelects(room: PublicRoom | null): void {
  if (!room?.settings) return;
  decorateTimeSelect.value = String(room.settings.decorateDurationMs);
  findTimeSelect.value = String(room.settings.findDurationMs);
}

function emitLobbySettings(): void {
  ...
  if (
    ...
    return;
  socket.emit("update_game_settings", getSelectedGameSettings());
}
...
function getPhaseMessage(phase?: GamePhase): string {
  if (phase === "role_assignment")
    return "역할을 배정하는 중입니다. 술래 1명과 도망자 여러 명으로 진행됩니다.";
  if (phase === "decorate")
    return "1분 안에 원을 자연스럽게 숨기고 제출하세요.";
  if (phase === "submit") return "다른 플레이어의 제출을 기다리는 중입니다.";
  if (phase === "reveal")
    return "제출 완료. 모든 도망자의 원이 공개되고 술래가 찾기를 준비합니다.";
  if (phase === "find")
    return "술래가 5분 안에 모든 도망자의 원을 찾는 단계입니다.";
  if (phase === "result")
    return "결과 단계입니다. 시간 초과면 도망자 승리, 모든 도망자를 잡으면 술래 승리입니다.";
...
async function drawLobbyRoomCanvas(room: PublicRoom): Promise<void> {
  ...
  lobbyCtx.fillText(
    `${room.playerCount}/${room.maxPlayers}명 연결됨 · ${getStateLabel(room.state)}`,
    48,
    96,
  );
}
...
function getSubmissionCircle(submission: ArtworkSubmission): {
  x: number;
  y: number;
  radius: number;
} {
  return {
    ...
    radius:
      submission.character.radiusRatio *
      Math.min(gameCanvas.width, gameCanvas.height),
  };
}
...
function getSubmissionImage(
  ...
  if (cached && cached.src === submission.paintDataUrl && cached.complete)
    return cached;

  const image =
    cached && cached.src === submission.paintDataUrl ? cached : new Image();
...
function updateSpyVisionOverlay(event?: PointerEvent): void {
  ...
  spyVisionOverlay.classList.toggle(
    "is-active",
    Boolean(active && progress > 0),
  );
  spyVisionOverlay.classList.toggle(
    "is-find",
    Boolean(active && phase === "find"),
  );
...
function getSubmissionAtPoint(point: {
  x: number;
  y: number;
  ...
  for (let index = submissions.length - 1; index >= 0; index -= 1) {
    ...
    if (
      submission.caught ||
      currentRoom?.game?.caughtTargetIds.includes(submission.playerId)
    )
      continue;
...
function renderGameCanvas(): void {
  ...
  if (currentRoom?.game && isRevealPhase(phase)) {
    ...
    currentRoom.game.submissions.forEach((submission) => {
      const selected =
        currentRoom?.game?.selectedTargetId === submission.playerId;
      ...
      const showHiddenGuide =
        phase === "result" || (!isSpy && phase === "reveal");
      drawSubmission(
        gameCtx,
        submission,
        selected,
...
function ensureCharacterInsideCanvas(): void {
  ...
  character.y = Math.min(
    gameCanvas.height - character.radius,
    Math.max(character.radius, character.y),
  );
}
...
function renderFocusScores(): void {
  ...
  const canViewFocus =
    isFindPhase && (me?.role === "spy" || me?.role === "artist");
...
function getPaintPoint(point: { x: number; y: number }): {
  x: number;
  y: number;
} {
  return {
    x:
      ((point.x - (character.x - character.radius)) / (character.radius * 2)) *
      paintCanvas.width,
    y:
      ((point.y - (character.y - character.radius)) / (character.radius * 2)) *
      paintCanvas.height,
  };
}
...
function paintAt(
  ...
  const localPreviousPoint = previousPoint
    ? getPaintPoint(previousPoint)
    : localPoint;
  const localBrushSize =
    brushSize * (paintCanvas.width / (character.radius * 2));
...
function sampleColor(point: { x: number; y: number }): void {
  if (!gameImage) return;
  drawBackgroundOnly(gameCtx);
  const pixel = gameCtx.getImageData(
    Math.floor(point.x),
    Math.floor(point.y),
    1,
    1,
...
function renderLobbyActions(room: PublicRoom | null): void {
  ...
  const canEditSettings = Boolean(room && isLobby && isHost) || !room;
  decorateTimeSelect.disabled = !canEditSettings;
  findTimeSelect.disabled = !canEditSettings;
  settingsHint.textContent = room
    ? `숨는 시간 ${formatMinutes(room.settings.decorateDurationMs)} / 찾는 시간 ${formatMinutes(room.settings.findDurationMs)} · 첫 술래 랜덤`
    : "방장이 선택한 시간으로 게임이 시작됩니다. 첫 술래는 랜덤입니다.";

  readyButton.disabled = !room || !isLobby || isHost;
  readyButton.textContent = isHost
    ? "방장은 자동 준비"
    : me?.ready
      ? "준비 취소"
      : "준비";
  ...
  startGameButton.classList.toggle(
    "start-active",
    Boolean(room && isLobby && isHost && room.canStart),
  );

  if (!room) {
    startHint.textContent = "방에 입장하면 참가자는 준비할 수 있습니다.";
  } else if (isHost && room.playerCount < room.maxPlayers) {
    startHint.textContent = `최소 1명이 입장하면 게임을 시작할 수 있습니다. 첫 술래는 랜덤입니다.`;
  } else if (isHost && !room.canStart) {
    startHint.textContent = `참가자 READY를 기다리는 중입니다. 첫 술래는 랜덤으로 정해집니다.`;
  } else if (isHost) {
    startHint.textContent = `준비 완료. 숨는 시간 ${formatMinutes(room.settings.decorateDurationMs)}, 찾는 시간 ${formatMinutes(room.settings.findDurationMs)}로 시작합니다.`;
  } else if (!me?.ready) {
    startHint.textContent =
      "참가자는 준비 버튼을 눌러야 합니다. 첫 술래는 게임 시작 시 랜덤으로 정해집니다.";
  } else {
    startHint.textContent =
...
function renderLobbyRoom(room: PublicRoom | null): void {
  ...
  artworkView.textContent = `${room.artwork.fileName} · ${formatBytes(room.artwork.sizeBytes)} · 숨기 ${formatMinutes(room.settings.decorateDurationMs)} / 찾기 ${formatMinutes(room.settings.findDurationMs)}`;
...
function scheduleAutoSubmit(room: PublicRoom): void {
  ...
  autoSubmitTimeout = window.setTimeout(() => {
    ...
    if (
      latestMe?.role === "artist" &&
      !latestMe.submitted &&
      (latestPhase === "decorate" || latestPhase === "submit")
    ) {
...
function getPhaseDurationMs(phase?: GamePhase): number {
  if (phase === "decorate" || phase === "submit")
    return (
      currentRoom?.settings.decorateDurationMs ?? DEFAULT_DECORATE_DURATION_MS
    );
  if (phase === "find")
    return currentRoom?.settings.findDurationMs ?? DEFAULT_FIND_DURATION_MS;
  return 0;
}
...
function renderGameTimer(): void {
  ...
  const text =
    duration > 0
      ? formatRemainingTime(getRemainingGameTime(currentRoom))
      : "대기";
  gameTimerView.textContent = text;
  topTimerView.textContent = text;

  if (
    (currentRoom.game.phase === "decorate" ||
      currentRoom.game.phase === "submit") &&
    duration > 0 &&
...
function renderGameRoom(room: PublicRoom): void {
  ...
  const canDecorate =
    me?.role === "artist" && (phase === "decorate" || phase === "submit");
  const canSubmit =
    me?.role === "artist" && (phase === "decorate" || phase === "submit");
  ...
  gameArtworkNameView.textContent =
    room.game?.artwork.fileName || room.artwork.fileName;
  ...
  submitArtworkButton.textContent = isSpy
    ? "술래 대기 중"
    : me?.submitted
      ? "제출 완료"
      : "제출";
  ...
  confirmFindButton.textContent =
    phase === "find" ? "클릭하면 잡힘 처리" : "찾기 단계 대기";
  restartGameButton.textContent = canRestart
    ? "다시 시작"
    : "RESULT 후 다시 시작";
  ...
  if (phase === "result" && room.game?.result) {
    ...
    submitStatusView.textContent = room.players
      .map(
        (player) =>
          `${player.name}: ${player.role === "spy" ? "술래 대기 중" : player.submitted ? "제출 완료" : "작성 중"}`,
      )
      .join(" / ");
  }
  ...
  toolHint.textContent = isSpy
    ? canFind
      ? "도망자는 실시간 점수를 보고, 술래는 제한 시간 안에 원을 찾습니다."
      : "술래는 꾸미기 도구를 사용할 수 없습니다. 제한 시간 안에 도망자의 원을 찾습니다."
    : isRevealPhase(phase)
      ? "제출 후에는 이동과 그리기가 잠깁니다. 술래의 선택을 기다립니다."
      : "기본은 항상 붓입니다. Space로 색을 가져오고 WASD로 이동합니다.";
...
function connectSocket(): void {
  ...
  socket.on("disconnect", () => {
    ...
    setMessage(
      "서버 연결이 끊겼습니다. 서버 실행 상태를 확인해 주세요.",
      "error",
    );
    lastRenderedRound = 0;
    renderRoom(null);
  ...
  socket.on(
    ...
    (payload: { round?: number; deadlineAt?: number }) => {
      ...
      if (
        ...
        submitCurrentArtwork();
        setMessage(
          "그리기 시간이 종료되어 현재 원 위치와 그림을 자동 제출했습니다.",
          "success",
        );
      }
    },
...
createRoomButton.addEventListener("click", () => {
  ...
  socket.emit("create_room", {
    ...
    settings: getSelectedGameSettings(),
  });
});
...
decorateTimeSelect.addEventListener("change", () => emitLobbySettings());
findTimeSelect.addEventListener("change", () => emitLobbySettings());
...
gameCanvas.addEventListener("pointerdown", (event) => {
  ...
  if (me?.role === "spy" && phase === "find") {
    ...
    if (target) {
      ...
      updateToolHint(
        "마우스 포인터로 아직 잡히지 않은 도망자 원 내부를 클릭해 주세요.",
      );
      renderGameCanvas();
    }
    return;
...
gameCanvas.addEventListener("pointermove", (event) => {
  ...
  const me = getMe(currentRoom);
  const phase = currentRoom?.game?.phase;
  if (me?.role === "spy" && (phase === "reveal" || phase === "find"))
    updateSpyVisionOverlay(event);
...
document.addEventListener("keydown", (event) => {
  ...
  if (moveKeys[key]) {
    ...
    character.x = Math.min(
      gameCanvas.width,
      Math.max(0, character.x + dx * speed),
    );
    character.y = Math.min(
      gameCanvas.height,
      Math.max(0, character.y + dy * speed),
    );
    flashHud(moveHud);
    renderGameCanvas();
    return;
  ...
  if (event.key === "[" || event.key === "]") {
    ...
    brushSize = Math.min(
      20,
      Math.max(1, brushSize + (event.key === "]" ? 1 : -1)),
    );
    brushSizeInput.value = String(brushSize);
    updateBrushStatus();
    flashHud(brushHud);
  ...
  if (key === "t") {
    ...
    flashHud(guideHud);
    updateToolHint(
      showCharacterGuide ? "내 원 위치 가이드 ON" : "내 원 위치 가이드 OFF",
    );
    renderGameCanvas();
    return;
  }
