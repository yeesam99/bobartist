...
## v0.0.61 Yacht Dice 클릭 안정성 및 다인 플레이 테스트
...
1. 채팅 메시지 전송 후 입력창을 다시 클릭하지 않고 연속 입력되는지 확인
2. `#/admin/chat` 접속 후 관리자 비밀번호 로그인 확인
3. BobArtist/Yacht Dice 채널이 게임과 방 코드별로 분리되는지 확인
4. 관리자 화면에서 새 메시지가 실시간으로 표시되는지 확인
5. 전체/채널 필터와 검색이 정상 동작하는지 확인
6. 잘못된 비밀번호가 거부되는지 확인

## v0.0.61 관리자 상태/접속 이력
- `#/admin/chat` 로그인 후 채팅 로그/게임 상태/플레이어 이력 탭 확인
- BobArtist/Yacht Dice 방 생성 시 게임 상태 탭 실시간 표시 확인
- 플레이어 닉네임을 바꿔 재접속했을 때 동일 Client ID의 닉네임 이력 누적 확인
- IP가 마스킹되어 표시되는지 확인
- 다른 브라우저/시크릿 창은 별도 Client ID로 기록되는지 확인
- 서버 재시작 후 메모리 이력이 초기화되는지 확인
