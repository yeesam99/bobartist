...
function joinRoom(socket: Socket, payload: ClientRoomRequest): void {
  ...
  if (!alreadyJoined && room.players.length >= MAX_PLAYERS) {
    socket.emit('room_error', { message: `방이 가득 찼습니다. 최대 ${MAX_PLAYERS}명까지 입장할 수 있습니다.` });
    return;
  }
...
function startGame(socket: Socket): void {
  ...
  if (!canStartGame(room)) {
    socket.emit('room_error', { message: '최소 2명 이상, 모든 도망자가 READY 상태여야 게임을 시작할 수 있습니다.' });
    return;
  }
  ...
  scheduleGameTimeout(room.code, room.game.round);
...
function restartGame(socket: Socket): void {
  ...
  scheduleGameTimeout(room.code, nextRound);
