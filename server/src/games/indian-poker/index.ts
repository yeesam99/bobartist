...
type PokerState = {
  ...
  lastAction: string; potResults: PotResult[]; tournamentWinnerId: string;
};
type PokerRoom = {
  code: string; hostId: string; state: "lobby" | "playing" | "revealed"; startingChips: number;
  players: PokerPlayer[]; game: PokerState | null; createdAt: number; updatedAt: number;
};
...
function toPublicGame(room: PokerRoom, viewerId: string) {
  ...
  return { roomCode:room.code,phase:room.game.phase,round:room.game.round,
    ...
    actedIds:[...room.game.actedIds],lastAction:room.game.lastAction,potResults:room.game.potResults.map((p)=>({...p,eligibleIds:[...p.eligibleIds],winnerIds:[...p.winnerIds]})),tournamentWinnerId:room.game.tournamentWinnerId };
}
...
function leaveRoom(io: Server, socket: Socket): void {
  ...
  if(room.state!=="lobby"){room.state="lobby";room.game=null;room.players=room.players.map((p)=>({...p,ready:p.isHost,chips:room.startingChips,...freshRoundFields()}));}
  room.updatedAt=Date.now();emitRoom(io,room);
}
function startRound(io: Server, room: PokerRoom): void {
  ...
  const first=room.players.find((p)=>cards[p.id]&&!p.allIn)?.id||"";
  room.state="playing";room.game={phase:"betting",round:(room.game?.round||0)+1,cards,winnerIds:[],pot,ante:ANTE,currentBet:0,currentTurnId:first,actedIds:[],lastAction:`Ante ${ANTE.toLocaleString()} Chips가 Pot에 모였습니다.`,potResults:[],tournamentWinnerId:""};
...
export function registerIndianPoker(io:Server):void{
  io.on("connection",(socket)=>{
    ...
    socket.on("indian-poker:create-room",(payload:PlayerRequest={})=>{leaveRoom(io,socket);const now=Date.now(),startingChips=normalizeStartingChips(payload.startingChips);const room:PokerRoom={code:createCode(),hostId:socket.id,state:"lobby",startingChips,players:[{id:socket.id,name:normalizeName(payload.playerName),ready:true,isHost:true,joinedAt:now,chips:startingChips,...freshRoundFields()}],game:null,createdAt:now,updatedAt:now};rooms.set(room.code,room);socket.join(`indian-poker:${room.code}`);socket.emit("indian-poker:room-created",toPublic(room));emitRoom(io,room);});
    ...
    socket.on("indian-poker:start-game",()=>{const room=findRoom(socket.id);if(!room)return emitError(socket,"참여 중인 방이 없습니다.");if(room.hostId!==socket.id)return emitError(socket,"방장만 게임을 시작할 수 있습니다.");if(!canStart(room))return emitError(socket,"2~6명이 참가할 수 있으며 모든 참가자가 Ready여야 합니다.");room.players=room.players.map((p)=>({...p,chips:room.startingChips,...freshRoundFields()}));startRound(io,room);});
